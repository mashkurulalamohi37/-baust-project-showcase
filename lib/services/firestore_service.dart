...
import '../mvc/models/activity.dart';
import '../mvc/models/review.dart';
import '../mvc/models/feedback.dart';
...
class FirestoreService {
  ...
  static Future<User?> getUser(String userId) async {
    try {
      ...
      return User(
        ...
        password: '', // Password not stored in Firestore for security
        role: userRole,
        createdAt: parseDate(safeData['createdAt']) ?? DateTime.now(),
        updatedAt: parseDate(safeData['updatedAt']) ?? DateTime.now(),
  ...
  static Future<void> createActivity(Activity activity) async {
    try {
      ...
      // Convert DateTime to Timestamp for Firestore (better for querying)
      data['timestamp'] = Timestamp.fromDate(activity.timestamp);
      await _firestore.collection(_activitiesCollection).doc(activity.id).set(data);
      print('FirestoreService: Activity created successfully: ${activity.title}');
    } catch (e) {
      print('ERROR: Failed to create activity: $e');
      print('ERROR: Stack trace: ${StackTrace.current}');
    }
  }

  static Future<List<Activity>> getRecentActivities({int limit = 10}) async {
    try {
      print('FirestoreService: Fetching recent activities (limit: $limit)');
      ...
      try {
        ...
        print('FirestoreService: Retrieved ${querySnapshot.docs.length} activities');
        ...
        final activities = querySnapshot.docs
            .map((doc) {
              try {
                ...
                print('ERROR: Failed to parse activity ${doc.id}: $e');
                return null;
              }
            })
        ...
        print('FirestoreService: Successfully parsed ${activities.length} activities');
        return activities;
      } catch (orderByError) {
        // If orderBy fails (e.g., missing index), try without orderBy and sort in memory
        print('WARNING: orderBy query failed, trying without orderBy: $orderByError');
      ...
      print('ERROR: Failed to get recent activities: $e');
      print('ERROR: Stack trace: ${StackTrace.current}');
      return [];
    }
  }
  ...
  static Future<void> logProjectUploaded(Project project) async {
    final activity = Activity(
      ...
      type: ActivityType.projectUploaded,
      title: 'New Project Uploaded',
      subtitle: '${project.authorName} uploaded "${project.title}"',
  ...
  static Future<void> logProjectStatusChange(
    ...
    switch (newStatus) {
      case ProjectStatus.approved:
        activityType = ActivityType.projectApproved;
        title = 'Project Approved';
        subtitle = '${changedByName ?? "Teacher"} approved "${project.title}"';
        break;
      case ProjectStatus.rejected:
        activityType = ActivityType.projectRejected;
        title = 'Project Rejected';
        subtitle = '${changedByName ?? "Teacher"} rejected "${project.title}"';
        break;
      case ProjectStatus.featured:
        activityType = ActivityType.projectFeatured;
        title = 'Project Featured';
        subtitle = '${changedByName ?? "Admin"} featured "${project.title}"';
        break;
      case ProjectStatus.needsRevision:
        activityType = ActivityType.projectNeedsRevision;
        title = 'Project Needs Revision';
        subtitle = '${changedByName ?? "Teacher"} requested revision for "${project.title}"';
        break;
      default:
  ...
  static Future<void> logProjectReviewed(Project project, String reviewerName) async {
    final activity = Activity(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      type: ActivityType.projectReviewed,
      title: 'Project Reviewed',
      subtitle: '$reviewerName reviewed "${project.title}"',
      timestamp: DateTime.now(),
      projectId: project.id,
      projectTitle: project.title,
  ...
  static Future<void> logTeacherRegistered(User teacher) async {
    final activity = Activity(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      type: ActivityType.teacherRegistered,
      title: 'New Teacher Registered',
      subtitle: '${teacher.name} registered for approval',
      timestamp: DateTime.now(),
      userId: teacher.id,
  ...
  static Future<void> logTeacherApproved(User teacher, String approvedBy) async {
    final activity = Activity(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      type: ActivityType.teacherApproved,
      title: 'Teacher Approved',
      subtitle: '${teacher.name} was approved',
      timestamp: DateTime.now(),
      userId: teacher.id,
      userName: teacher.name,
  ...
  static Future<void> logTeacherRejected(User teacher, String rejectedBy) async {
    final activity = Activity(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      type: ActivityType.teacherRejected,
      title: 'Teacher Rejected',
      subtitle: '${teacher.name} was rejected',
      timestamp: DateTime.now(),
      userId: teacher.id,
      userName: teacher.name,
