...
class _SemesterArchiveScreenState extends State<SemesterArchiveScreen> {
  ...
  Widget build(BuildContext context) {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildSelectionCard(
                        ...
                        () => _selectSubmissionType(
                            ProjectSubmissionType.academic),
    ...
    final keys = groupedProjects.keys.toList()
      ..sort((a, b) {
        ...
        return (partsA[0] == 'Summer' ? 1 : 0)
            .compareTo(partsB[0] == 'Summer' ? 1 : 0);
      });
  ...
  Widget _buildSelectionCard(String title, IconData icon, Gradient gradient,
...
class _SemesterProjectsScreenState extends State<SemesterProjectsScreen> {
  ...
  Future<void> _exportProjects(String filter) async {
    ...
    try {
      ...
      debugPrint(
          'SemesterProjectsScreen: Found ${allProjects.length} total projects, filtering for $filter for ${widget.semesterName} ${widget.year} -> ${projectsToExport.length} projects to export');

      if (projectsToExport.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('No projects found matching criteria')),
          );
        }
        return;
      }
  ...
  ({Color color, IconData icon, Gradient gradient}) _getAwardInfo(
      ProjectAward award) {
    switch (award) {
